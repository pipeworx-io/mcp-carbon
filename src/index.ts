...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Carbon');
}
...
const tools: McpToolExport['tools'] = [
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        postcode: { type: 'string', description: 'UK outcode (first part of a postcode), e.g. "RG10". No spaces.' },
        regionid: { type: 'number', description: 'GB region id 1–17 (e.g. 13 = London).' },
      },
    },
  },
  {
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    ...
    case 'get_regional_intensity':
      return getRegionalIntensity(args);
    case 'get_intensity_factors':
      return ciJson('/intensity/factors');
    default:
...
async function getIntensity() {
  const res = await pwFetch(`${BASE_URL}/intensity`, {
    headers: { Accept: 'application/json' },
  });
  if (!res.ok) throw await httpError(res, 'Carbon Intensity API error');
...
async function getIntensityByDate(date: string) {
  ...
  const res = await pwFetch(`${BASE_URL}/intensity/date/${encodeURIComponent(date)}`, {
    headers: { Accept: 'application/json' },
  });
  if (!res.ok) throw await httpError(res, 'Carbon Intensity API error');
...
async function getGenerationMix() {
  const res = await pwFetch(`${BASE_URL}/generation`, {
    headers: { Accept: 'application/json' },
  });
  if (!res.ok) throw await httpError(res, 'Carbon Intensity API error');
...
async function ciJson(path: string): Promise<unknown> {
  const res = await pwFetch(`${BASE_URL}${path}`, { headers: { Accept: 'application/json' } });
  if (!res.ok) throw await httpError(res, 'Carbon Intensity API error');
  return res.json();
}

async function getRegionalIntensity(args: Record<string, unknown>) {
  const postcode = typeof args.postcode === 'string' ? args.postcode.trim() : '';
  if (postcode) return ciJson(`/regional/postcode/${encodeURIComponent(postcode)}`);
  if (args.regionid !== undefined && args.regionid !== null && `${args.regionid}`.trim() !== '') {
    const n = Number(args.regionid);
    if (!Number.isFinite(n)) throw new Error('"regionid" must be a number 1–17.');
    return ciJson(`/regional/regionid/${encodeURIComponent(String(Math.trunc(n)))}`);
  }
  return ciJson('/regional');
}
